...
function displayPage(pageNumber) {
  ...
  for (let i = startIndex; i < endIndex && i < contacts.length; i++) {
      ...
      // Populate contactItem with contact data
...
function createPaginationLinks() {
  for (let i = 1; i <= totalPages; i++) {
      ...
      pageLinkAnchor.addEventListener('click', () => {
          currentPage = i;
          displayPage(i);
...
function updatePaginationLinks() {
  const paginationLinks = paginationList.querySelectorAll('li');
...
// Initially, display the first page of contact items and create pagination links
displayPage(1);
